...
boot_alloc(uint32_t n)
{
	...
	else
	{
		...
		{
			nextfree = result;
			panic("Boot alloc: insufficient memory");
		}
	}
...
mem_init(void)
{
	...
	boot_map_region(kern_pgdir, (uintptr_t) envs, NENV * sizeof(struct Env), PADDR(envs), PTE_U | PTE_P);
	//////////////////////////////////////////////////////////////////////
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, KERNBASE, 268435456, 0, PTE_P | PTE_W);
...
page_init(void)
{
	...
	size_t i;
	for (i = 1; i < npages; i++) {
		if(page2pa(&pages[i]) < (IOPHYSMEM) || page2pa(&pages[i]) >= (EXTPHYSMEM + PTSIZE))
		{
			pages[i].pp_ref = 0;
...
page_free(struct PageInfo *pp)
{
	...
	if(pp->pp_ref != 0)
	{
		panic("page_free: page reference field non-zero");
	}
	if(pp->pp_link != NULL)
	{
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	// Fill this function in
	pte_t *p_pte;
	for(int i = 0; (i+1) * PGSIZE <= size; i++)
	{
		p_pte = pgdir_walk(pgdir, (char*)va + (i*PGSIZE), 1);
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	{
		if((uintptr_t) curr >= ULIM)
		{
			user_mem_check_addr = (uintptr_t) curr;
			return -E_FAULT;
		}
		pte_t * page = pgdir_walk(env->env_pgdir, curr, 0);
		cprintf("page: %x\n", page);
		if(!page || (*page & perm) != perm)
		{
			user_mem_check_addr = (uintptr_t) curr;
			return -E_FAULT;
		}
		if((uintptr_t) curr % PGSIZE != 0)
		{
			curr = ROUNDDOWN(curr, PGSIZE);
		}
	}
